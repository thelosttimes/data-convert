...
import com.my.dataconvert.exception.MigrationException;
import com.my.dataconvert.manager.JdbcManager;
import com.my.dataconvert.netty.Client;
import com.my.dataconvert.netty.pool.NettyInstance;
import com.my.dataconvert.utils.Config;
import com.my.dataconvert.utils.InetInfo;
...
public class MigrationTask implements Runnable {
	...
	public void run() {
		...
		while(loop) {
			...
			log.info("迁移" + (flag ? "成功" : "失败") + ",花费时间:" + (System.currentTimeMillis() - start_time) 
					+ ",表名:" + table + ",limit:" + min + "-" + table_pageSize);
	...
	public boolean reg_device() {
		...
		if(null == bodies || bodies.length != 7) {
